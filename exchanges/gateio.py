...
logger = logging.getLogger("cex_dex_arbitrage.exchanges.gateio")

# class GateIo(ExchangeFetcher):
#     def __init__(self, pair: str):
#         super().__init__("GateIo", pair)
#         self.exchange = ccxt.pro.gateio()

#     async def connect(self):
#         async def listener():
#             while True:
#                 try:
#                     # logger.debug(f"[GateIo] Connecting WebSocket for {self.pair}...")
#                     ticker = await self.exchange.watch_ticker(self.pair)
#                     self.latest_price = ticker["last"]
#                     self.connected = True
#                     # logger.debug(f"[GateIo] Latest price for {self.pair}: {self.latest_price}")
#                 except Exception as e:
#                     self.connected = False
#                     logger.error(f"[WebSocket] GateIo error ({self.pair}): {e}")
...
#             logger.debug(f"[GateIo] Order book for {self.pair}:")
#             logger.debug(f"\tAsks: {order_book['asks'][:5]}")
#             logger.debug(f"\tBids: {order_book['bids'][:5]}")
#             return order_book
#         except Exception as e:
#             logger.error(f"[WebSocket] GateIo order book error ({self.pair}): {e}")
#             logger.debug(traceback.format_exc())
#             return None
...
class GateIo(ExchangeFetcher):
    ...
    async def connect(self):
        """Continuously listen to GateIo WebSocket and update latest prices."""
        async def listener():
            while True:
                try:
                    # Bulk‐subscribe once
                    ...
                    self.connected = False
                    logger.error(f"[WebSocket] GateIo error: {e}")
                    logger.debug(traceback.format_exc())
                    await asyncio.sleep(self._reconnect_interval)
